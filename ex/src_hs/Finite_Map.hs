...
delFromFM (Branch key elt size fm_l fm_r) del_key =
  if del_key > key then mkBalBranch key elt fm_l (delFromFM fm_r del_key)
  else if del_key < key then mkBalBranch key elt (delFromFM fm_l del_key) fm_r
...
intersectFM_C combiner fm1 (Branch split_key elt2 _ left right)
  ...
  where
    ...
    elt1  = case maybe_elt1 of Just x -> x
